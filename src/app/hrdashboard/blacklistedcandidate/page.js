...
const BlacklistedCandidate = () => {
  ...
  useEffect(() => {
    const sorted = [...allData].sort((a, b) => {
      const dateA = new Date(a.Blacklist_Date);
      const dateB = new Date(b.Blacklist_Date);
  ...
  const filteredData = sortedData.filter((data) => {
    ...
    const lowercaseData = {
      ...
      Blacklist_Date: (data.Blacklist_Date || "").toLowerCase(),
    };
    ...
    return (
      ...
      lowercaseData.Blacklist_Date.includes(searchString)
    );
  });
              ...
              <tbody>
                {paginatedData.map((data, index) => (
                  <tr key={index} onClick={() => handleRowClick(data)}>
                    ...
                    <td style={{ fontSize: "14px" }}>
                      {new Date(data.Blacklist_Date).toLocaleDateString()}
                    </td>
                    <td style={{ fontSize: "14px" }}>
                      <button
                        type="button"
                  ...
                  {selectedRowData && (
                    <div>
                      ...
                      <p>
                        Blacklist Date:{" "}
                        {new Date(
                          selectedRowData.Blacklist_Date
                        ).toLocaleDateString()}
                      </p>
                      <p>
                        Blacklist Description:{" "}
                        {selectedRowData.Blacklist_Description}
                      </p>
                      <p>
                        Applied Date:{" "}
                        {new Date(
...
export default BlacklistedCandidate;
